# 06 — Household View (Janey + Michael)

Retirement is a **household** outcome, so this pulls Janey and Michael together.
Figures from the **Household** tab of
[`janeys-pension-projections.xlsx`](../janeys-pension-projections.xlsx).
Personal identifiers (account numbers, NI, contact details) are kept out of this repo.

## What we have

| | Janey | Michael |
|--|-------|---------|
| Age | 53 (DOB 05/11/1972) | 57 (DOB 02/07/1969) |
| Target retirement | 65 | 65 (assumed) |
| Employment | Homemaker | One Air Ltd, OPS — salary ~£171k |
| State Pension | £11,973/yr from age 67 — **05/11/2039** | **£241/wk ≈ £12,532/yr** from age 67 — **~02/07/2036** |
| DB / service pension | **Teachers': £7,085/yr** (£6,982 from age 60) + £20,945 lump sum | **RAF AFPS 75: ~£5,340/yr** from age 60 + ~£16,020 lump sum (estimate) |
| Private pension pot today | ~£0 (starting the £3,600/yr) | **£221,974** (Aegon SIPP) ✅ |
| Ongoing contributions | £3,600/yr | **~£32,490/yr** (19% of salary via sacrifice) |
| Projected pot at 65 | ~£58,200 | **~£634,140** |

> **State Pension dates (from confirmed DOBs):** Michael (02/07/1969) reaches State
> Pension age 67 in **~July 2036**; Janey (05/11/1972) in **Nov 2039**. (The earlier
> "Nov 2039" was Janey's date.) Amounts still worth confirming at gov.uk/check-state-pension;
> dates don't change the steady-state total below, only the bridge timing.
>
> **Contribution figure:** £32,490/yr = the employer pension line (£2,707.50/mo, 19% of
> salary, which under salary sacrifice includes his 10% sacrifice). If the £1,425/mo
> sacrifice is *on top* of that, total is £49,590/yr and the pot/income are higher still.
>
> **Michael's RAF pension (AFPS 75):** RAF service 1990–98 (8 yrs, Sergeant, aircrew).
> Left before the 22-year point → a **preserved pension payable from age 60** + a tax-free
> lump sum (~3× the pension), index-linked. ~8/22 of a Sergeant's 22-year pension
> (~£14,684) ≈ **£5,340/yr**, but **aircrew rates were higher, so likely more** — get an
> official forecast from Veterans UK / Defence Business Services.
>
> **Not yet added:** Michael's newer **NEST pot** current value (separate from the
> £221,974 Aegon SIPP) — would push the numbers higher again.

## Household income — steady state (both retired, both State Pensions in payment)

| Component | Janey | Michael | Household |
|-----------|-------|---------|-----------|
| Private pension (drawdown ~4%) | ~£2,327 | ~£25,366 | ~£27,693 |
| DB / service pension (Teachers' · RAF) | £6,982 | ~£5,340 | ~£12,322 |
| State Pension | £11,973 | £12,532 | £24,505 |
| **Total annual income** | **~£21,282** | **~£43,238** | **~£64,519** |
|  |  | **≈ per month** | **~£5,377** |

**Plus one-off tax-free lump sums around age 60:** Janey ~£20,945 (Teachers') + Michael
~£16,020 (RAF) ≈ **~£37,000**, on top of the 25% tax-free cash from the DC pots.

## Against the PLSA couple benchmarks (today's money)

| Standard | Couple / year | Household position |
|----------|---------------|--------------------|
| **Minimum** | ~£22,400 | ✅ **~£42,120 above** |
| **Moderate** | ~£43,100 | ✅ **~£21,420 above** |
| **Comfortable** | ~£59,000 | ✅ **~£5,520 above** |

*PLSA couple figures are illustrative — verify at
[retirementlivingstandards.org.uk](https://www.retirementlivingstandards.org.uk/).*

## What this tells us

1. **The household clears "Comfortable" with room to spare.** ~£64,520/year (~£5,377/month),
   ~£5,500 above the PLSA "Comfortable" couple standard — and that's *before* Michael's NEST
   pot and on the conservative contribution reading (£32,490/yr). At £49,590/yr it's higher again.
2. **Four guaranteed, inflation-linked income streams** underpin it: two State Pensions
   (~£24,500), Janey's Teachers' Pension (~£7,000) and Michael's RAF pension (~£5,340) =
   ~£36,800/year secure and index-linked — ~57% of the household total before any investment
   risk. Unusually resilient.
3. **Michael's DC pot is the growth engine.** ~£222k + ~£32,490/year for 8 years → ~£634,000
   by 65, giving ~£25,400/year of drawdown — the single largest stream.
4. **Two service pensions + lump sums land at age 60.** Janey's Teachers' and Michael's RAF
   pensions (£6,982 + £5,340 ≈ £12,300/year combined) plus ~£37,000 of tax-free lump sums are
   payable from age 60 — so the years from 60 to State Pension age are very well covered.
5. **Watch the Annual Allowance.** Michael's ~£32,490/year is within the £60,000 allowance,
   and the taper doesn't bite (threshold income ~£171k, under £200k). Keep an eye on it if
   salary, bonuses or contributions rise — carry-forward of unused allowance can help.
6. **Tax in retirement.** At ~£43k income Michael would pay basic/higher-rate tax on his
   drawdown; the PLSA figures are indicative rather than after-tax. Sequencing withdrawals
   (tax-free cash and both people's personal allowances) improves the net result.
7. **The £500k mortgage-free home is a backstop** — upside for "Comfortable-plus", care
   costs, or a legacy, not needed for the core plan.

## To refine further — Michael's remaining numbers

- [x] Salary and contributions — **added** (~£171k; ~£32,490/yr into pension)
- [ ] Confirm the contribution total (£32,490 vs £49,590 — see note above)
- [ ] **NEST pot current value** → add to his existing pot (Inputs B40)
- [x] State Pension dates resolved from DOBs (Michael ~Jul 2036, Janey Nov 2039)
- [ ] Confirm the **State Pension amounts** (forecasts at gov.uk/check-state-pension)
- [ ] Confirm his **target retirement age** (assumed 65) → Inputs B36

## Year-by-year timeline (see the **Timeline** tab)

Household income (today's money, indicative) as each stream switches on:

| Year | Mike / Jane age | Event | Household /yr |
|------|-----------------|-------|---------------|
| 2029 | 60 / 57 | Mike's RAF pension + **£16,020 lump sum** | ~£5,340 |
| 2032 | 63 / 60 | Jane's Teachers' pension + **£20,945 lump sum** | ~£12,322 |
| 2034 | 65 / 62 | Mike retires, draws DC pot | ~£37,687 |
| 2036 | 67 / 64 | Mike's State Pension starts | ~£50,219 |
| 2037 | 68 / 65 | Jane retires, draws DC pot | ~£52,546 |
| 2039 | 70 / 67 | Jane's State Pension starts — **all streams on** | **~£64,519** |

The key point: **both service pensions and both tax-free lump sums arrive from age 60**,
so there's no real income gap before the State Pensions begin — each transition steps
income *up*. Household income climbs from ~£5,340 (2029) to its ~£64,519 steady state
by 2039, comfortably above "Comfortable" throughout the fully-retired years.

*(Today's-money/real terms; the guaranteed pensions are index-linked. The DC pots' 25%
tax-free cash is additional to the lump sums shown. First year of each stream may be
part-year.)*
